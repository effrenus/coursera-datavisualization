...
function getKanjiLevel(kanji) {
	for (let i = 0; i < kanjiJLPT.length; i++) {
		if (kanjiJLPT[i].indexOf(kanji) !== -1) {
			return i + 1;
		}
	}
	return 0;
...
d3.json('/kanji_grade.json', function(err, data) {
	let grades = new Array(11).join(',').split('').map((d, i) => {
		return {
			n: i + 1,
			count: 0,
			levels: [0, 0, 0, 0, 0, 0]
	...
	data.forEach((item) => {
		...
		let i = grade - 1;
		item.jlpt = getKanjiLevel(item.literal);
		grades[i].count += 1;
		grades[i].levels[item.jlpt] += 1;
	});
	...
	x.domain(grades.map(d => {
		return `${d.n}`;
	}));
	y.domain([0, d3.max(grades, d => {
		return d.count;
	})]);
	...
	color.domain(['N0', 'N1', 'N2', 'N3', 'N4', 'N5']);

	let gradeEl = svg.selectAll('.grade')
		.data(grades)
		.enter()
		.append('g').attr('class', 'g')
		.attr('transform', function(d) {
			return 'translate(' + x(d.n) + ', 0)';
		});

	gradeEl.selectAll('rect')
		.data(d => {
			return d.levels;
		})
		.enter().append('rect')
